...
BEGIN
	...
	SELECT @NGVL = INSERTED.NGVL FROM INSERTED
	SELECT @NGSINH = NHANVIEN.NGSINH FROM INSERTED INNER JOIN NHANVIEN ON INSERTED.MANV = NHANVIEN.MANV
	IF(@NGVL < @NGSINH)
	BEGIN
...
BEGIN
	...
	DECLARE @NGKT SMALLDATETIME
	SELECT @THOIGIAN = INSERTED.THOIGIAN FROM INSERTED
	SELECT @NGBD = DEAN.NGBD FROM INSERTED INNER JOIN DEAN ON DEAN.MADA = INSERTED.MADA
	SELECT @NGBD = DEAN.NGKT FROM INSERTED INNER JOIN DEAN  ON DEAN.MADA = INSERTED.MADA
	IF(@NGBD > @THOIGIAN)
	BEGIN
		PRINT N'THỜI GIAN PHÂN CÔNG PHẢI NẰM TRONG THỜI GIAN ĐỀ ÁN!!!'
		ROLLBACK TRANSACTION
	END
	IF(@NGKT < @THOIGIAN)
	BEGIN
...
INSERT INTO PHANCONG(MANV, MADA, THOIGIAN) VALUES('N003', 'DA001', '19/3/2020')

INSERT INTO PHANCONG(MANV, MADA, THOIGIAN) VALUES('N002', 'DA002', '17/3/2019')
